...
import type {
  ScanHistoryProps as BaseScanHistoryProps,
  ScanHistoryEntry,
} from "../../types/barcode";
  ...
  return (
    <div className="scan-history-row flex flex-col bg-[var(--card-bg)] dark:bg-[var(--card-bg)] rounded-2xl px-4 py-3 shadow-lg justify-between transition-all duration-300 w-full">
      <div className="flex flex-col items-start flex-1 min-w-0 w-full">
        ...
        <span
          className="font-mono text-lg md:text-xl select-all text-left break-all text-indigo-900 dark:text-indigo-100 bg-white/70 dark:bg-zinc-800/70 px-3 py-2 rounded-lg shadow-sm whitespace-nowrap overflow-x-auto w-full max-w-full"
          style={{
...
export default function ScanHistory({
      ...
      const imageUrls = await Promise.all(
        matchingFiles.map(async (fileRef) => {
          try {
            ...
            console.error(
              `Error getting download URL for ${fileRef.name}:`,
              error,
            );
            return null;
          }
  ...
  const handleExport = useCallback(() => {
    ...
    notify?.(
      `${numericCodes.length} códigos numéricos exportados exitosamente`,
      "green",
    );
  }, [history, notify]);
  ...
  return (
    <div className="space-y-6 p-4 md:p-6 rounded-3xl shadow-2xl bg-[var(--card-bg)] dark:bg-[var(--card-bg)] border border-[var(--input-border)] scan-history-container backdrop-blur-xl w-full overflow-x-auto">
      <div className="flex items-center justify-between mb-6 md:mb-8">
        <h3 className="text-lg font-bold text-center flex-1 text-indigo-700 dark:text-indigo-200">
        ...
        <div className="flex gap-2 ml-2">
          ...
          <button
            ...
            onClick={() => {
              if (
                window.confirm(
                  "¿Seguro que deseas borrar todo el historial de escaneos?",
                )
              ) {
          ...
          <div className="bg-[var(--card-bg)] rounded-lg w-full h-full max-w-none max-h-none overflow-hidden flex flex-col">
            ...
            <div className="px-6 py-3 border-b border-gray-200 dark:border-gray-600 flex-shrink-0">
              <p className="text-gray-600 dark:text-gray-300 text-sm">
                Código:{" "}
                <span className="font-mono bg-[var(--input-bg)] px-3 py-1 rounded text-base">
                  {currentImageCode}
                </span>
              </p>
            </div>
          ...
          >
            <div className="relative w-[90%] h-[90%] flex items-center justify-center">
              ...
              <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 z-10 px-4 py-2 rounded-full bg-black bg-opacity-70 text-white text-sm">
                Código: {currentImageCode} • Imagen {selectedImageIndex + 1} de{" "}
                {codeImages.length}
              </div>
            </div>
          </div>,
