...
export default function SessionCounter({
  ...
  const [isMinimized, setIsMinimized] = useState(() => {
    ...
    return localStorage.getItem("session-counter-minimized") === "true";
  });
  ...
  useEffect(() => {
    const updateTimer = () => {
      ...
      const warningThreshold = useTokenAuth
        ? 24 * 60 * 60 * 1000
        : 30 * 60 * 1000; // 24 horas vs 30 minutos
  ...
  const isWarning =
    sessionTimeLeft <= warningThreshold && sessionTimeLeft > criticalThreshold;
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div
                  className={`time-display text-lg leading-tight ${isCritical ? "animate-pulse" : ""}`}
                >
                  {timeLeft}
                </div>
                {!isMinimized && (
