...
const TIPOS_EGRESO_XML: TipoEgresoXml[] = (
  tiposEgresoXmlCatalog as TipoEgresoXml[]
)
  .filter(
    (t) => t && typeof t.codigo === "string" && typeof t.nombre === "string",
  )
  .map((t) => ({
...
function inferTipoComprobanteFromRootLocalName(localName: string | null): {
  codigo?: string;
  nombre?: string;
} {
...
function parseFacturaXml(xmlText: string): FacturaInfo {
  ...
  const monedaEl = resumenEl
    ? firstElement(resumenEl, "CodigoTipoMoneda")
    : null;

  const computeTotalDescuentos = (): string | undefined => {
    // Primary (FE CR): <ResumenFactura><TotalDescuentos>
    const fromResumen = resumenEl
      ? firstText(resumenEl, "TotalDescuentos")
      : undefined;
  ...
  if (resumenEl) {
    const desgloseEls = resumenEl.getElementsByTagNameNS(
      "*",
    ...
    for (let i = 0; i < desgloseEls.length; i++) {
      ...
      const key =
        codigo && codigoTarifaIVA ? `${codigo}|${codigoTarifaIVA}` : "";
  ...
  const info: FacturaInfo = {
    ...
    tiposTransaccion: tipoTransaccionSet.size
      ? Array.from(tipoTransaccionSet)
      : undefined,
    ...
    resumen: resumenEl
      ? {
          ...
          totalMercanciasGravadas: firstText(
            resumenEl,
          ...
          desgloseImpuesto: desgloseImpuesto.length
            ? desgloseImpuesto
            : undefined,
        }
      : undefined,
...
export default function XmlPage() {
  ...
  const [tipoEgresoQueryByItemId, setTipoEgresoQueryByItemId] = useState<
    Record<string, string>
  >({});
  const [openTipoEgresoDropdownItemId, setOpenTipoEgresoDropdownItemId] =
  ...
  const [confirmAction, setConfirmAction] = useState<
    "clear" | "export" | "delete" | "deleteReceptor" | null
  >(null);
  ...
  const [openReceptorGroupByKey, setOpenReceptorGroupByKey] = useState<
  ...
  const pickFilesLabel =
    files.length > 0 ? "Agregar más archivos" : "Seleccionar XML";
      ...
      try {
        ...
        const message =
          err instanceof Error
            ? err.message
            : "Error desconocido parseando XML";
        next.push({
          id: rec.fileName,
          fileName: rec.fileName,
          status: "error",
        ...
        for (const file of xmlFiles) {
          try {
            ...
            const message =
              err instanceof Error
                ? err.message
                : "Error desconocido cargando XML";
  ...
  const performRemoveOne = useCallback(
    async (id: string) => {
      try {
        await remove(id);
  ...
  const performRemoveMany = useCallback(
    async (ids: string[]) => {
  ...
  const onRequestRemoveReceptor = useCallback(
    (group: {
      key: string;
      receptorId: string;
      receptorLabel: string;
  ...
  const receptorGroups = useMemo(() => {
    ...
    for (const item of items) {
      const receptorId = (
        item.factura?.receptor?.identificacionNumero || ""
      ).trim();
      const receptorName = (
        item.factura?.receptor?.nombre ||
        item.factura?.receptor?.nombreComercial ||
        ""
      ).trim();
      ...
      if (existing) {
        ...
        if (!existing.receptorName && receptorName)
          existing.receptorName = receptorName;
      } else {
        map.set(key, {
          key,
          receptorId,
    ...
    groups.sort((a, b) => {
      ...
      return (a.receptorLabel || "").localeCompare(
        b.receptorLabel || "",
        "es",
  ...
  const receptorCedulaGroupCount = receptorGroups.reduce(
    (acc, g) => acc + ((g.receptorId || "").trim() ? 1 : 0),
    0,
  );
  ...
  const performExportExcel = useCallback(
    async (options?: {
      records?: XmlDbRecord[];
      allowMissingTipo?: boolean;
    }) => {
      if (!isReady) return;
      if (dbError) {
      ...
      const records =
        options?.records ?? ((await getAllFromDb()) as XmlDbRecord[]);
      if (records.length === 0) {
      ...
      if (missingTipo.length > 0 && !allowMissingTipo) {
        showToast(
          `Faltan tipos de egreso en ${missingTipo.length} XML. Asigna el tipo antes de exportar.`,
          "warning",
        );
        return;
      }
      ...
      if (missingTipo.length > 0 && allowMissingTipo) {
        showToast(
          `Exportando con faltantes: ${missingTipo.length} XML quedarán como “SIN TIPO”.`,
          "warning",
        );
      }
      ...
      try {
        okItems = records.map((r) => {
          ...
          const tipoLabel = tipo
            ? tipoEgresoCodigoToLabel.get(tipo) || tipo
            : "SIN TIPO";
      ...
      type TaxKey = string;
      const toTaxKey = (codigo?: string, codigoTarifaIVA?: string): TaxKey =>
        `${(codigo || "").trim()}|${(codigoTarifaIVA || "").trim()}`;
      ...
      for (const item of okItems) {
        ...
        for (const d of desglose) {
          ...
          if (!taxKeyToLabel.has(key)) {
            ...
            const tarifa = d.codigoTarifaIVA
              ? labelForCodigoTarifaIVA(d.codigoTarifaIVA)
              : "—";
      ...
      for (const item of okItems) {
        ...
        const totalVenta = applySign(
          parseDecimal(f.resumen?.totalVenta) || 0,
          sign,
        );
        const totalDescuentos = applySign(
          parseDecimal(f.resumen?.totalDescuentos) || 0,
          sign,
        );
        const otrosCargos = applySign(
          parseDecimal(f.resumen?.totalOtrosCargos) || 0,
          sign,
        );
        const totalComprobante = applySign(
          parseDecimal(f.resumen?.totalComprobante) || 0,
          sign,
        );
        ...
        sumByTipoEgreso.set(
          tipoKey,
          (sumByTipoEgreso.get(tipoKey) || 0) + totalComprobante,
        );
        ...
        for (const d of desglose) {
          ...
          if (isNotaCredito) {
            sumNotasCreditoByTaxKey.set(
              key,
              (sumNotasCreditoByTaxKey.get(key) || 0) + signedMonto,
            );
          } else {
            sumFacturasByTaxKey.set(
              key,
              (sumFacturasByTaxKey.get(key) || 0) + signedMonto,
            );
          }
        }
      ...
      const tipoEgresoEntries = Array.from(sumByTipoEgreso.entries()).sort(
        (a, b) => {
          const va = a[1];
          const vb = b[1];
      ...
      rows.push([
        ...
        "",
        "",
        ...taxKeys.map((k) => sumFacturasByTaxKey.get(k) || 0),
        "",
        sumFacturasTotalComprobante,
      ]);
      rows.push([
        "TOTAL NOTAS DE CRÉDITO (NC)",
        "",
        "",
        "",
        "",
        "",
        ...taxKeys.map((k) => sumNotasCreditoByTaxKey.get(k) || 0),
        "",
        sumNotasCreditoTotalComprobante,
      ]);
      rows.push([
      ...
      try {
        ...
        const msg =
          err instanceof Error ? err.message : "Error exportando a Excel";
        showToast(msg, "error");
      }
    },
    [
      ...
      dbError,
      getAllFromDb,
      tipoEgresoCodigoToLabel,
      tipoEgresoCodigoToCuenta,
      clearAll,
      showToast,
    ],
  );
      ...
      try {
        ...
        const body = okItems.map((item) => {
          ...
          addToSum(
            totalSumByCurrency,
            moneda,
            f.resumen?.totalComprobante,
            sign,
          );

          if (isNotaCredito) {
            addToSum(
              ivaNotasCreditoSumByCurrency,
              moneda,
              f.resumen?.totalImpuesto,
              sign,
            );
            addToSum(
              totalNotasCreditoSumByCurrency,
              moneda,
              f.resumen?.totalComprobante,
              sign,
            );
            ...
            addToSum(
              ivaFacturasSumByCurrency,
              moneda,
              f.resumen?.totalImpuesto,
              sign,
            );
            addToSum(
              totalFacturasSumByCurrency,
              moneda,
              f.resumen?.totalComprobante,
              sign,
            );
          ...
          const tipoKey = item.tipoEgresoLabel;
          const tipoMap =
            sumByTipoEgreso.get(tipoKey) || new Map<string, number>();
          ...
          for (const d of desglose) {
            ...
            const byCurrency =
              ivaSumsByTaxKey.get(key) || new Map<string, number>();
            byCurrency.set(
              curr,
              (byCurrency.get(curr) || 0) + applySign(monto, sign),
            );
            ivaSumsByTaxKey.set(key, byCurrency);
          }

          const iva = formatMoneyForPdf(
            String(
              applySign(parseDecimal(f.resumen?.totalImpuesto) || 0, sign),
            ),
            moneda,
          );
          const descuento = formatMoneyForPdf(
            String(
              applySign(parseDecimal(f.resumen?.totalDescuentos) || 0, sign),
            ),
            moneda,
          );
          const total = formatMoneyForPdf(
            String(
              applySign(parseDecimal(f.resumen?.totalComprobante) || 0, sign),
            ),
            moneda,
          );
        ...
        body.push([
          "",
          "",
          "TOTALES (FACTURAS / NOTAS DE CRÉDITO / NETO)",
          "",
          "",
          "",
          "",
        ...
        body.push([
          ...
          "",
          "TOTAL FACTURAS",
          formatTotalsForPdf(ivaFacturasSumByCurrency),
          "",
          formatTotalsForPdf(totalFacturasSumByCurrency),
          "",
          "",
        ]);
        body.push([
          ...
          "",
          "TOTAL NOTAS DE CRÉDITO (NC)",
          formatTotalsForPdf(ivaNotasCreditoSumByCurrency),
          "",
          formatTotalsForPdf(totalNotasCreditoSumByCurrency),
          "",
          "",
        ]);
        body.push([
        ...
        autoTable(doc, {
          ...
          didParseCell: (data: any) => {
            ...
            if (
              ...
              rowIndex === summaryTitleRowIndex &&
              ivaTaxKeyToLabel.size > 0
            ) {
              data.cell.styles.fontStyle = "bold";
            }
            ...
            if (
              typeof rowIndex === "number" &&
              rowIndex === tipoEgresoTitleRowIndex
            ) {
              data.cell.styles.fontStyle = "bold";
            }
            ...
            if (
              typeof rowIndex === "number" &&
              rowIndex === splitTotalsTitleRowIndex
            ) {
              data.cell.styles.fontStyle = "bold";
            }
          },
        ...
        const msg =
          err instanceof Error ? err.message : "Error exportando a PDF";
        showToast(msg, "error");
      }
    },
    [
  ...
  const onExport = useCallback(() => {
    void (async () => {
      ...
      if (receptorGroups.length > 1) {
        showToast(
          "No se puede exportar cuando hay más de un receptor. Deja solo un receptor cargado para exportar.",
          "warning",
        );
        return;
      }
    ...
    <div className="max-w-6xl mx-auto space-y-6 px-3 sm:px-4 lg:px-0">
      <ConfirmModal
        ...
        message={
          confirmAction === "clear" ? (
            ...
            <>
              ¿Seguro que deseas eliminar este XML? Se borrará de los XML
              almacenados.
        ...
        actionType={
          confirmAction === "clear"
            ...
            : confirmAction === "export"
              ? "change"
              : confirmAction === "delete" || confirmAction === "deleteReceptor"
                ? "delete"
                : "assign"
        }
          ...
          void (async () => {
            ...
            try {
              if (action === "clear") {
                ...
                pendingExportAssignedRecordsRef.current =
                  pendingExportRecordsRef.current ?? null;
      ...
      {exportOptionsOpen && (
        <div
          ...
          style={{ pointerEvents: "auto" }}
          onMouseDown={(e) => {
            if (e.target === e.currentTarget && !exportOptionsLoading)
                  ...
                  void (async () => {
                    ...
                    try {
                      await performExportPdf({
                        records:
                          pendingExportAssignedRecordsRef.current ?? undefined,
            ...
            <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
              <div>
                ...
                <p className="text-sm text-[var(--muted-foreground)] mt-1">
                  Carga uno o varios XML. Se lee toda la factura excepto
                  productos (DetalleServicio/LineaDetalle).
                </p>
              </div>
              <div className="flex flex-col sm:flex-row gap-2 w-full lg:w-auto">
                ...
                {(() => {
                  ...
                  return (
                    <div
                      className={`relative group w-full sm:w-52 ${exportBlockedByReceptor ? "cursor-not-allowed" : ""}`}
                    >
                      <button
                        type="button"
                        onClick={onExport}
                      ...
                      {exportBlockedByReceptor && (
                        <div className="hidden group-hover:block absolute z-30 left-1/2 -translate-x-1/2 top-full mt-2 w-max max-w-[18rem]">
                          <div className="rounded border border-yellow-400 bg-yellow-200 text-red-700 text-xs px-3 py-2 shadow-lg">
                            No se puede exportar porque hay más de un receptor
                            cargado.
                          </div>
                        </div>
                      )}
                      ...
                      <div className="mt-2 grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
                        <div className="text-[var(--muted-foreground)] md:col-span-3">
                          <div className="flex items-start ">
                            <span className="font-medium text-[var(--foreground)] block sm:inline">
                              Clave:
                            </span>
                        ...
                        {f?.resumen?.totalDescuentos &&
                          parseDecimal(f.resumen.totalDescuentos) !== null &&
                          parseDecimal(f.resumen.totalDescuentos) !== 0 && (
                            <div className="text-[var(--muted-foreground)]">
                              <span className="font-medium text-[var(--foreground)]">
                                Descuento:
                    ...
                    <div className="relative w-full sm:w-[28rem] lg:w-[32rem] max-w-full">
                      <input
                        value={(() => {
                          ...
                          const selectedLabel = selectedCodigo
                            ? tipoEgresoCodigoToLabel.get(selectedCodigo) ||
                              selectedCodigo
                            : "";
                          return (
                            tipoEgresoQueryByItemId[item.id] ?? selectedLabel
                          );
                        })()}
                        onChange={(e) => {
                        ...
                        onFocus={(e) => {
                          ...
                          setTipoEgresoQueryByItemId((prev) => ({
                            ...prev,
                            [item.id]: prev[item.id] ?? selectedLabel,
                          }));
                        ...
                        onBlur={() => {
                          ...
                          setTimeout(() => {
                            setOpenTipoEgresoDropdownItemId((prev) =>
                      ...
                      {openTipoEgresoDropdownItemId === item.id &&
                        (() => {
                          const query = (tipoEgresoQueryByItemId[item.id] || "")
                            .trim()
                            .toLowerCase();
                                          ...
                                          void (async () => {
                                            try {
                                              ...
                                              const msg =
                                                err instanceof Error
                                                  ? err.message
                                                  : "Error guardando tipo de egreso";
                                              showToast(msg, "error");
                                            }
                                          })();
                                          setTipoEgresoQueryByItemId(
                      ...
                      <div className="p-4 rounded border border-[var(--border)] bg-[var(--muted)]">
                        ...
                        <div className="text-sm text-[var(--muted-foreground)] space-y-1">
                          ...
                          <div>
                            ...
                            {f.codigoActividadEmisor || "—"}
                          </div>
                          <div>
                            <span className="font-medium text-[var(--foreground)]">
                              Actividad receptor:
                            </span>{" "}
                            {f.codigoActividadReceptor || "—"}
                          </div>
                          <div>
                            <span className="font-medium text-[var(--foreground)]">
                              Condición venta:
                          ...
                          <div>
                            ...
                            {f.tiposTransaccion && f.tiposTransaccion.length > 0
                              ? f.tiposTransaccion
                                  .map((t) =>
                                    labelForCode(t, TIPO_TRANSACCION_LABEL),
                                  )
                                  .join(" | ")
                              : "—"}
                          </div>
                      ...
                      <div className="p-4 rounded border border-[var(--border)] bg-[var(--muted)]">
                        ...
                        <div className="text-sm text-[var(--muted-foreground)] space-y-1">
                          ...
                          <div>
                            ...
                            {formatMoney(
                              f.resumen?.totalMercanciasGravadas,
                              f.resumen?.moneda,
                            )}
                          </div>
                          <div>
                          ...
                          <div>
                            ...
                            {formatMoney(
                              f.resumen?.totalVentaNeta,
                              f.resumen?.moneda,
                            )}
                          </div>
                          {f.resumen?.totalOtrosCargos &&
                            parseDecimal(f.resumen.totalOtrosCargos) !== null &&
                            parseDecimal(f.resumen.totalOtrosCargos) !== 0 && (
                              <div>
                                <span className="font-medium text-[var(--foreground)]">
                                  Total otros cargos:
                        ...
                        <div className="mt-3">
                          ...
                          <div className="text-sm text-[var(--muted-foreground)] space-y-1">
                            ...
                            {(() => {
                              const desgloseRaw =
                                f.resumen?.desgloseImpuesto || [];
                              const desglose = desgloseRaw.filter((d) => {
                                const monto = parseDecimal(
                                  d.totalMontoImpuesto,
                                );
                                return (
                                  monto === null || Math.abs(monto) >= 1e-9
                                );
                              });
                              ...
                              if (desglose.length === 0) {
                                return (
                                  <div className="text-xs text-[var(--muted-foreground)]">
                                    Sin desglose de IVA en el XML.
                                  </div>
                                );
                              }

                              const renderRow = (
                                d: (typeof desglose)[number],
                                idx?: number,
                              ) => (
                                <div
                                  key={idx}
                        ...
                        {f.resumen?.mediosPago &&
                          f.resumen.mediosPago.length > 0 && (
                            <div className="mt-3">
                              <div className="font-medium text-[var(--foreground)] mb-1">
                                Medios de pago
                              </div>
                                ...
                                {f.resumen.mediosPago.map((mp, idx) => (
                                  ...
                                  >
                                    ...
                                    <div>
                                      ...
                                      {formatMoney(mp.total, f.resumen?.moneda)}
                                    </div>
                                  </div>
                                ))}
                              </div>
                      ...
                      <div className="p-4 rounded border border-[var(--border)] bg-[var(--muted)]">
                        ...
                        <div className="text-sm text-[var(--muted-foreground)] space-y-1">
                          ...
                          <div>
                            ...
                            {(f.emisor?.identificacionTipo || "—") +
                              " " +
                              (f.emisor?.identificacionNumero || "")}
                          </div>
                          <div>
                            <span className="font-medium text-[var(--foreground)]">
                              Nombre comercial:
                            </span>{" "}
                            {f.emisor?.nombreComercial || "—"}
                          </div>
                          <div>
                            <span className="font-medium text-[var(--foreground)]">
                              Correo:
                            </span>{" "}
                            {f.emisor?.correoElectronico || "—"}
                          </div>
                          <div>
                            <span className="font-medium text-[var(--foreground)]">
                              Teléfono:
                      ...
                      <div className="p-4 rounded border border-[var(--border)] bg-[var(--muted)]">
                        ...
                        <div className="text-sm text-[var(--muted-foreground)] space-y-1">
                          ...
                          <div>
                            ...
                            {(f.receptor?.identificacionTipo || "—") +
                              " " +
                              (f.receptor?.identificacionNumero || "")}
                          </div>
                          <div>
                            <span className="font-medium text-[var(--foreground)]">
                              Nombre comercial:
                            </span>{" "}
                            {f.receptor?.nombreComercial || "—"}
                          </div>
                          <div>
                            <span className="font-medium text-[var(--foreground)]">
                              Correo:
                            </span>{" "}
                            {f.receptor?.correoElectronico || "—"}
                          </div>
                          <div>
                            <span className="font-medium text-[var(--foreground)]">
                              Teléfono:
          ...
          if (receptorGroups.length === 1) {
            return (
              <div className="space-y-4">
                {receptorGroups[0]!.items.map(renderItem)}
              </div>
            );
          }
                  ...
                  <div key={group.key} className="space-y-3">
                    <div className="bg-[var(--card-bg)] border border-[var(--input-border)] rounded-lg shadow p-4 sm:p-5">
                      <div className="flex items-start justify-between gap-3">
                        ...
                        >
                          <ChevronDown
                            className={`w-4 h-4 mt-0.5 transition-transform ${isOpen ? "rotate-180" : ""}`}
                          />
                          <div className="min-w-0">
                            <div className="font-semibold text-[var(--foreground)] truncate">
                        ...
                        <div className="flex flex-col sm:flex-row items-end sm:items-center gap-2 flex-shrink-0">
                          ...
                          {canDeleteReceptorBlocks &&
                            Boolean((group.receptorId || "").trim()) && (
                              <button
                                type="button"
                                onClick={(e) => {
                                  e.preventDefault();
