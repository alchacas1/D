    ...
    try {
      ...
      if (saveConfirmState.mode === "edit" && saveConfirmState.productId) {
        await updateProducto(
          saveConfirmState.productId,
          saveConfirmState.input,
        );
        showToast("Producto actualizado.", "success");
        setLastSaveFeedback({
          type: "edit",
          nombre: saveConfirmState.input.nombre,
        });
      } else {
  ...
  return (
    <div className="max-w-5xl mx-auto bg-[var(--card-bg)] border border-[var(--input-border)] rounded-lg shadow p-4 sm:p-6">
      <ConfirmModal
        ...
        confirmText={
          saveConfirmState?.mode === "edit" ? "Actualizar" : "Guardar"
        }
        cancelText="Cancelar"
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-4">
        ...
        <EmpresaSearchAddSection
          ...
          searchPlaceholder={
            productosLoading ? "Cargando..." : "Buscar producto"
          }
                  ...
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="text-right shrink-0">
                      ...
                      <div className="mt-1 text-[10px] sm:text-xs text-[var(--muted-foreground)] whitespace-nowrap">
                        {formatNumber(p.pesoengramos, 0)} g • ₡{" "}
                        {formatNumber(p.precioxgramo, 2, 2)}/g
                      </div>
                    </div>
                  </div>
                </div>
