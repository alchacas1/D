...
export default function TicketCarousel({
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === "ArrowLeft")
        setCurrent((c) => (c > 0 ? c - 1 : tickets.length - 1));
      if (e.key === "ArrowRight")
        setCurrent((c) => (c < tickets.length - 1 ? c + 1 : 0));
    };
  ...
  const sorteosUnicos = useMemo(
    () => Array.from(new Set(tickets.map((t) => t.sorteo))),
    [tickets],
  );
  ...
  if (total === 0)
    return (
      <div className="w-full flex flex-col items-center text-[var(--tab-text)] py-8">
        No hay tickets.
      </div>
    );
    ...
    <div className="w-full flex flex-col items-center">
      ...
      <div className="flex flex-row gap-2 mb-3 w-full max-w-xs items-center justify-center">
        <div className="flex-1 flex items-center gap-2">
          <span className="inline-flex items-center justify-center w-8 h-8 text-[var(--tab-text)]">
      ...
      {!editModalOpen &&
        ...
        (!showAll ? (
          <div
            ...
            style={{ height: `calc(${cardH} + 2.5rem)`, minHeight: "7.5rem" }}
          >
            {filteredTickets.map((ticket, idx) => {
              ...
              return (
                <div
                  ...
                  style={{
                    zIndex,
                    transform: `translate(-50%, -50%) translateX(${translateX}px) translateY(${translateY}px) scale(${scale * (holdingId === ticket.id ? 0.9 : 1)}) rotate(${rotate}deg)`,
                ...
                >
                  <div className="flex justify-between items-center mb-1">
                    <span
                      className="font-bold text-xs uppercase tracking-wide break-words whitespace-pre-line w-4/5"
                      style={{
                        color: "var(--foreground)",
                  ...
                  <div className="flex-1 flex flex-col justify-center items-center">
                    <span className="font-mono text-lg text-green-700 font-bold">
            ...
            {filteredTickets.map((ticket) => (
              <div
                ...
                style={{
                  ...
                  transition: "all 0.3s cubic-bezier(.4,2,.6,1)",
                  transform:
                    holdingId === ticket.id ? "scale(0.90)" : "scale(1)",
                }}
                onMouseDown={() => {
                  setHoldingId(ticket.id);
                  if (holdTimeout.current) clearTimeout(holdTimeout.current);
          ...
          <div className="bg-[var(--card-bg)] rounded-lg shadow-xl p-6 w-full max-w-xs mx-4">
            ...
            <div className="mb-4">
              ...
              <div className="font-mono text-green-700">
                ₡ {editTicket.amount.toLocaleString("es-CR")}
              </div>
              <div className="text-xs opacity-70">{editTicket.time}</div>
            </div>
            <div className="flex gap-2">
              <button
                className="flex-1 py-2 px-4 rounded bg-gray-200 dark:bg-gray-700"
                onClick={() => {
              ...
              <button
                className="flex-1 py-2 px-4 rounded bg-blue-600 text-white"
                onClick={() => {
                  setShowEditConfirm(false);
                  setEditModalOpen(true);
                  setEditFields({
                    sorteo: editTicket.sorteo,
                    amount: editTicket.amount,
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex items-center justify-center">
          <div className="bg-[var(--card-bg)] rounded-lg shadow-xl p-6 w-full max-w-xs mx-4">
            ...
            <div className="mb-4 space-y-2">
              <div>
                ...
                <input
                  ...
                  value={editFields.sorteo}
                  onChange={(e) =>
                    setEditFields((f) => ({ ...f, sorteo: e.target.value }))
                  }
                />
              </div>
              <div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setEditFields((f) => ({ ...f, time: e.target.value }))
                  }
                />
              </div>
            </div>
            <div className="flex gap-2">
              ...
              <button
                className="flex-1 py-2 px-4 rounded bg-green-600 text-white"
                onClick={() => {
