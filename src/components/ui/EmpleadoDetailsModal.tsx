...
function normalizeExtraList(
  list: Array<{ pregunta?: unknown; respuesta?: unknown }>,
): ExtraQA[] {
  return list
    .map((x) => ({
...
function RequiredMark({ show, isEmpty }: { show: boolean; isEmpty: boolean }) {
  ...
  return (
    <span
      className={isEmpty ? "text-red-500" : "text-[var(--muted-foreground)]"}
    >
      {" "}
      *
    </span>
...
export default function EmpleadoDetailsModal({
  ...
  const [baseSnapshot, setBaseSnapshot] = useState<ComparableState | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    setBaseSnapshot({
      ...
      diaContratacion: baseDynamicIngresoDate
        ? ""
        : String(empleado?.diaContratacion || "").trim(),
      ...
      contratoFisico: normalizeYesNo(
        String(empleado?.contratoFisico || ""),
      ).trim(),
      espacioComida: normalizeYesNo(
        String(empleado?.espacioComida || ""),
      ).trim(),
      brindanVacaciones: normalizeYesNo(
        String(empleado?.brindanVacaciones || ""),
      ).trim(),
      incluidoCCSS: baseIncluidoCCSS,
      incluidoINS: baseIncluidoINS,
      preguntasExtra: normalizeExtraList(
        Array.isArray(empleado?.preguntasExtra)
          ? empleado!.preguntasExtra!
          : [],
      ),
    });
    ...
    setBrindanVacaciones(
      normalizeYesNo(String(empleado?.brindanVacaciones || "")),
    );
    setIncluidoCCSS(Boolean(empleado?.incluidoCCSS));
    setIncluidoINS(Boolean(empleado?.incluidoINS));
    setPreguntasExtra(
      Array.isArray(empleado?.preguntasExtra)
        ? empleado!.preguntasExtra!.map((x) => ({
  ...
  const displayedDiaContratacion =
    String(diaContratacion || "").trim() ||
  ...
  const currentSnapshot: ComparableState = useMemo(
    () => ({
      ...
      diaContratacion: useDynamicIngresoDate
        ? ""
        : String(diaContratacion || "").trim(),
  ...
  const validate = (): { ok: boolean; msg?: string } => {
    ...
    if (!dia && !useDynamicIngresoDate)
      return { ok: false, msg: "Día de contratación es obligatorio." };

    if (!String(paganAguinaldo || "").trim())
      return { ok: false, msg: "Pagan aguinaldo es obligatorio." };

    if (!isCcssType(ccssType))
      return { ok: false, msg: "Tipo de jornada (TC/MT/PH) es obligatorio." };

    if (!String(danReciboPago || "").trim())
      return { ok: false, msg: "Le dan recibo de pago es obligatorio." };
    if (!String(contratoFisico || "").trim())
      return { ok: false, msg: "Contrato físico es obligatorio." };
    if (!String(espacioComida || "").trim())
      return {
        ok: false,
        msg: "Se cuenta con espacio de comida es obligatorio.",
      };
    if (!String(brindanVacaciones || "").trim())
      return { ok: false, msg: "Se brindan vacaciones es obligatorio." };
    ...
    for (const [idx, qa] of preguntasExtra.entries()) {
      ...
      if (!q)
        return {
          ok: false,
          msg: `Pregunta extra #${idx + 1}: la pregunta es obligatoria.`,
        };
      if (!a)
        return {
          ok: false,
          msg: `Pregunta extra #${idx + 1}: la respuesta es obligatoria.`,
        };
    }
          ...
          <div className="p-4 sm:p-6">
            <div className="flex items-start justify-between gap-2 mb-4 sm:mb-6">
              <div className="min-w-0 flex-1">
                ...
                <div className="text-xs text-[var(--muted-foreground)] mt-1 break-words">
                  {readOnly ? "Solo lectura" : "Campos obligatorios"} · Empresa
                  ID: {String(empleado.empresaId || "")}
                </div>
              </div>
              <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="md:col-span-2">
                <label className="block text-sm font-medium text-[var(--foreground)] mb-1">
                  Cantidad de horas que trabaja a la semana?
                  <RequiredMark
                    show={!readOnly}
                    isEmpty={!String(ccssType || "").trim()}
                  />
                </label>
                {readOnly ? (
              ...
              <div className="md:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="bg-[var(--card-bg)] border border-[var(--input-border)] rounded-md px-3 py-2">
                  <label className="block text-sm text-[var(--foreground)] mb-1">
                    ...
                    <RequiredMark
                      show={!readOnly}
                      isEmpty={!paganAguinaldo.trim()}
                    />
                  </label>
                  {readOnly ? (
                    <span className="text-sm text-[var(--muted-foreground)]">
                      {strLabel(empleado.paganAguinaldo)}
                    </span>
                  ) : (
                    <AutoResizeTextarea
                ...
                <div className="bg-[var(--card-bg)] border border-[var(--input-border)] rounded-md px-3 py-2">
                  <label className="block text-sm text-[var(--foreground)] mb-1">
                    Se da recibo de pago?
                    <RequiredMark
                      show={!readOnly}
                      isEmpty={!danReciboPago.trim()}
                    />
                  </label>
                  {readOnly ? (
                    <span className="text-sm text-[var(--muted-foreground)]">
                      {strLabel(empleado.danReciboPago)}
                    </span>
                  ) : (
                    <AutoResizeTextarea
                      value={danReciboPago}
                ...
                <div className="bg-[var(--card-bg)] border border-[var(--input-border)] rounded-md px-3 py-2">
                  <label className="block text-sm text-[var(--foreground)] mb-1">
                    Se cuenta con contrato físico?
                    <RequiredMark
                      show={!readOnly}
                      isEmpty={!contratoFisico.trim()}
                    />
                  </label>
                  {readOnly ? (
                    <span className="text-sm text-[var(--muted-foreground)]">
                      {strLabel(empleado.contratoFisico)}
                    </span>
                  ) : (
                    <AutoResizeTextarea
                ...
                <div className="bg-[var(--card-bg)] border border-[var(--input-border)] rounded-md px-3 py-2">
                  <label className="block text-sm text-[var(--foreground)] mb-1">
                    ...
                    <RequiredMark
                      show={!readOnly}
                      isEmpty={!espacioComida.trim()}
                    />
                  </label>
                  {readOnly ? (
                    <span className="text-sm text-[var(--muted-foreground)]">
                      {strLabel(empleado.espacioComida)}
                    </span>
                  ) : (
                    <AutoResizeTextarea
                      value={espacioComida}
                ...
                <div className="bg-[var(--card-bg)] border border-[var(--input-border)] rounded-md px-3 py-2">
                  <label className="block text-sm text-[var(--foreground)] mb-1">
                    Se brindan vacaciones?
                    <RequiredMark
                      show={!readOnly}
                      isEmpty={!brindanVacaciones.trim()}
                    />
                  </label>
                  {readOnly ? (
                    <span className="text-sm text-[var(--muted-foreground)]">
                      {strLabel(empleado.brindanVacaciones)}
                    </span>
                  ) : (
                    <AutoResizeTextarea
                ...
                <div className="bg-[var(--card-bg)] border border-[var(--input-border)] rounded-md px-3 py-2">
                  ...
                  {readOnly ? (
                    <span className="text-sm text-[var(--muted-foreground)]">
                      {boolLabel(empleado.incluidoCCSS)}
                    </span>
                  ) : (
                    <input
                      type="checkbox"
                      checked={incluidoCCSS}
                      onChange={(e) => setIncluidoCCSS(e.target.checked)}
                      className="w-5 h-5"
                    />
                  )}
                </div>
                ...
                <div className="bg-[var(--card-bg)] border border-[var(--input-border)] rounded-md px-3 py-2">
                  ...
                  {readOnly ? (
                    <span className="text-sm text-[var(--muted-foreground)]">
                      {boolLabel(empleado.incluidoINS)}
                    </span>
                  ) : (
                    <input
                      type="checkbox"
                      checked={incluidoINS}
                      onChange={(e) => setIncluidoINS(e.target.checked)}
                      className="w-5 h-5"
                    />
                  )}
                </div>
            ...
            <div className="mt-6">
              ...
              {(() => {
                const items = readOnly
                  ? preguntasExtra.filter(
                      (x) =>
                        String(x?.pregunta || "").trim() ||
                        String(x?.respuesta || "").trim(),
                    )
                  : preguntasExtra;
                ...
                return (
                  <div className="mt-3 space-y-3">
                    {items.map((qa, idx) => (
                      <div
                        key={idx}
                        className="grid grid-cols-1 gap-3 bg-[var(--card-bg)] border border-[var(--input-border)] rounded-md p-3"
                      >
                        {readOnly ? (
