...
export function SupplierWeekSection(props: SupplierWeekSectionProps) {
  ...
  const selectedCompanyForUi = (
    companySelectorValue ?? companyForProviders
  ).trim();
  ...
  const selectedProviderName = useMemo(() => {
    ...
    return (
      eligibleProviders.find((p) => p.code === selectedProviderCode)?.name || ""
    );
    ...
    >
            ...
            <button
              ...
              onClick={() => {
                ...
                const selectedCompany = (
                  companyForProviders || selectedCompanyForUi
                ).trim();

                if (selectedCompany) {
                  try {
                    const oldValue =
                      localStorage.getItem(sharedCompanyKey) ?? "";
      ...
      <ConfirmModal
        ...
        message={
          <div>
            <div>
              ¿Deseas eliminar el monto asignado
              {selectedProviderName ? " para " : ""}
