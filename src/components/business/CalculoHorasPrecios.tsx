...
export default function CalculoHorasPrecios() {
  ...
  const [pricePerHourStatus, setPricePerHourStatus] = useState<
    "loading" | "config" | "default"
  >("default");

  const [timeData, setTimeData] = useState<{
    [employeeName: string]: {
      [day: string]: { seconds: number; timeHHMMSS: string };
    };
  }>({});
  ...
  const monthName = useMemo(
    () =>
      currentDate.toLocaleDateString("es-CR", {
        month: "long",
        year: "numeric",
      }),
    [currentDate],
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (!user) {
          ...
          owned = (allEmpresas || []).filter((e) => {
            ...
            const ownerIdMatch =
              e.ownerId &&
              resolvedOwnerId &&
              String(e.ownerId) === String(resolvedOwnerId);
            const ownerCompanieMatch =
              assignedEmpresa &&
              (String(e.name) === String(assignedEmpresa) ||
                String(e.ubicacion) === String(assignedEmpresa));
        ...
        if (user?.role === "admin") {
          const allowed = (userPermissions.scanhistoryEmpresas || [])
        ...
        const mapped: MappedEmpresa[] = (owned || []).map((e) => ({
          ...
          names: (e.empleados || [])
            .filter(
              (emp) =>
                Boolean((emp as any)?.amboshorarios) ||
                Boolean((emp as any)?.calculoprecios),
            )
        ...
        if (!empresa) {
          if (isUserRole && assignedEmpresa) {
            ...
            storedEmpresa &&
            mapped.some((m) => String(m.value) === String(storedEmpresa))
          ) {
  ...
  const names = useMemo(
    () => empresas.find((e) => e.value === empresa)?.names || [],
    [empresas, empresa],
  );
  ...
  useEffect(() => {
    ...
    const loadValorHora = async () => {
      ...
      try {
        ...
        const configs =
          await CcssConfigService.getAllCcssConfigsByOwner(empresaOwnerId);
        const match = (configs || [])
          .flatMap((cfg) => (cfg as any)?.companie || [])
          .find((c: any) =>
            candidates.includes(normalizeKey(c?.ownerCompanie)),
          );
  ...
  useEffect(() => {
    const loadTime = async () => {
      ...
      if (isUserRole && assignedEmpresa) {
        const assignedStr = String(assignedEmpresa).toLowerCase();
        const currentEmpresaLabel = (
          empresas.find((e) => e.value === empresa)?.label || ""
        ).toLowerCase();
        const currentEmpresaValue = String(empresa).toLowerCase();
        const ok =
          currentEmpresaValue === assignedStr ||
          currentEmpresaLabel === assignedStr ||
          currentEmpresaLabel.includes(assignedStr);
        if (!ok) {
      ...
      try {
        const entries = await CalculoHorasService.getEntriesByLocationMonth(
          empresa,
          year,
          month,
        );
  ...
  const openHoursModal = (employeeName: string, day: number) => {
    const currentTimeHHMMSS =
      timeData[employeeName]?.[String(day)]?.timeHHMMSS || "00:00:00";
    setModal({ isOpen: true, employeeName, day, currentTimeHHMMSS });
  };

  const handleSaveTime = async (payload: {
    timeHHMMSS: string;
    totalSeconds: number;
  }) => {
    ...
    try {
      setSaving(true);
      await CalculoHorasService.upsertTime(
        empresa,
        modal.employeeName,
        year,
        month,
      ...
      setTimeData((prev) => {
        ...
        if (payload.totalSeconds <= 0) {
          ...
          next[employeeName][dayKey] = {
            seconds: payload.totalSeconds,
            timeHHMMSS: payload.timeHHMMSS,
          };
        }
        return next;
      });
      ...
      <div className="flex items-center justify-center p-8 bg-[var(--card-bg)] rounded-lg border border-[var(--input-border)]">
        <div className="text-center">
          ...
          <p className="text-[var(--muted-foreground)]">
            No tienes permisos para acceder a Cálculo horas precios.
          </p>
        </div>
      </div>
    );
            ...
            <div className="flex-1 min-w-0 sm:min-w-[220px]">
              ...
              <select
                ...
                onChange={(e) => {
                  if (isUserRole) {
                    showToast(
                      "No tienes permisos para cambiar de empresa",
                      "error",
                    );
                    return;
                  }
              ...
              <tbody>
                {(selectedEmployee === "Todos"
                  ...
                  <tr key={name}>
                    ...
                    >
                      {(() => {
                        const totalSeconds = daysToShow.reduce(
                          (acc, day) =>
                            acc + (timeData[name]?.[String(day)]?.seconds || 0),
                          0,
                        );
                    ...
                    {daysToShow.map((day) => {
                      const seconds =
                        timeData[name]?.[String(day)]?.seconds || 0;
                      const timeHHMMSS =
                        timeData[name]?.[String(day)]?.timeHHMMSS || "";
                      const isToday =
                        isCurrentMonthView && day === todayInfo.day;
                      return (
                        <td
                          key={day}
                        ...
                        >
                          <button
                            ...
                            title={
                              seconds > 0
                                ? `${timeHHMMSS} - Clic para editar`
                                : "Clic para agregar tiempo"
                            }
                          >
