...
export default function TimingControl() {
  ...
  const [ticketToDelete, setTicketToDelete] = useState<TicketEntry | null>(
    null,
  );
  ...
  const resumenSorteos = tickets.reduce(
    (acc, ticket) => {
  ...
  const exportToPNG = async () => {
    if (!personName.trim()) {
      toast.showToast(
        "Por favor ingresa el nombre de la persona antes de exportar",
        "warning",
      );
      return;
    }
  ...
  const handleConfirmExport = async () => {
    ...
    try {
      ...
      toast.showToast(
        `Imagen exportada exitosamente. QR generado para descarga móvil.`,
        "success",
      );
    } catch (error) {
      console.error("Error al exportar:", error);
      toast.showToast(
        "Error al exportar la imagen. Por favor intenta de nuevo.",
        "error",
      );
    } finally {
  ...
  const handleEditTicket = (editedTicket: TicketEntry) => {
    setTickets((prev) =>
      prev.map((t) =>
        t.id === editedTicket.id ? { ...t, ...editedTicket } : t,
      ),
    );
  };
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      switch (e.key) {
        ...
        case "ArrowLeft":
          ...
          setSelectedSorteoIndex((prev) => (prev <= 0 ? maxIndex : prev - 1));
          break;
      ...
      <React.Fragment>
        ...
        {showSummary && (
          <div className="fixed inset-0 bg-black bg-opacity-60 z-[9999] flex items-center justify-center p-4">
            <div
              className="rounded-2xl shadow-2xl p-6 sm:p-8 w-[96vw] max-w-2xl mx-auto relative"
              style={{
            ...
            >
              ...
              {Object.keys(resumenSorteos).length === 0 ? (
                <div
                  ...
                  style={{ color: "var(--foreground)" }}
                >
                  No hay sorteos con monto asignado.
                </div>
              ) : (
        ...
        {showCodeModal && (
          <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
            <div
              className="rounded-2xl shadow-xl p-4 sm:p-6 w-[95vw] max-w-2xl mx-auto max-h-[90vh] overflow-y-auto relative"
              style={{
                background: "var(--card-bg)",
                color: "var(--foreground)",
              }}
            >
              ...
              <div
                className="text-xs text-center mb-4 p-2 rounded"
                style={{
        ...
        {showDeleteModal && ticketToDelete && (
          <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
            <div
              className="rounded-2xl shadow-xl p-4 sm:p-6 w-[95vw] max-w-md mx-auto relative"
              style={{
                background: "var(--card-bg)",
                color: "var(--foreground)",
              }}
        ...
        {showQRModal && (
          <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
            <div
              className="rounded-2xl shadow-2xl p-6 sm:p-8 w-[96vw] max-w-md mx-auto relative"
              style={{
                background: "var(--card-bg)",
                color: "var(--foreground)",
            ...
            >
              ...
              <div className="text-center">
                ...
                >
                  Escanea este código QR con tu móvil para descargar la imagen
                  automáticamente
                </p>
                ...
                >
                  La imagen permanecerá disponible en el almacenamiento en la
                  nube
                </p>
              </div>
            </div>
            ...
            <div className="flex-1 min-w-0 max-w-full flex flex-col">
              ...
              {keyBuffer && !isMobile && (
                <div className="mb-4 export-hide">
                  ...
                  >
                    Escribiendo:{" "}
                    <span className="ml-2 font-bold">{keyBuffer}</span>
                  </div>
                </div>
              )}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  ...
                  <button
                    ...
                    onClick={() => {
                      if (
                        window.confirm(
                          "¿Seguro que deseas limpiar todos los tickets y datos guardados?",
                        )
                      ) {
                ...
                !showQRModal && (
                  <div className="mb-6">
                    ...
                    <TicketCarousel
                      ...
                      onDelete={(ticket) =>
                        handleDeleteTicket({ ...ticket, code: "" })
                      }
                      onEdit={(edited) =>
                        handleEditTicket({
                          ...edited,
                          code:
                            tickets.find((t) => t.id === edited.id)?.code || "",
                        })
                      }
                    />
                  </div>
                  ...
                  <div className="mb-6 text-center py-12">
                    ...
                    >
                      <div className="mb-4">
                        ...
                        >
                          Comienza agregando tickets de tus sorteos para llevar
                          un control de ventas
                        </p>
                      </div>
