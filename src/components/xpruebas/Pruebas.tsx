...
const summarizeCompanyMovements = (
  ...
  movements.forEach((movement) => {
    const currency =
      (movement as { currency?: MovementCurrencyKey }).currency === "USD"
        ? "USD"
        : "CRC";
    if (currency === "USD") {
      totalUSD += 1;
...
const FIRESTORE_COLLECTIONS: Array<{
  name: string;
  label: string;
  notes?: string;
}> = [
  ...
  {
    name: "MovimientosFondos",
    label: "MovimientosFondos",
    notes: "Incluye subcolección movements",
  },
];
...
export default function Pruebas() {
  ...
  const [movimientosCompanies, setMovimientosCompanies] = useState<
    MovimientosCompanyRecord[]
  >([]);
  ...
  const [exportingCompanyId, setExportingCompanyId] = useState<string | null>(
    null,
  );
  const [importingCompanyId, setImportingCompanyId] = useState<string | null>(
    null,
  );
  const [movimientosCounts, setMovimientosCounts] = useState<
  ...
  const fetchMovimientosCompanies = useCallback(async () => {
    ...
    try {
      const { MovimientosFondosService } =
        await import("@/services/movimientos-fondos");
  ...
  const handleExportMovimientos = useCallback(
    async (docId: string, companyLabel: string) => {
      ...
      const readableLabel =
        companyLabel && companyLabel.trim().length > 0 ? companyLabel : docId;
      ...
      try {
        ...
        const summary = summarizeCompanyMovements(
          bundle.ledger,
          bundle.movements,
        );
  ...
  const handleImportMovimientosCompany = useCallback(
    async (docId: string, companyLabel: string) => {
        ...
        fileInput.onchange = async (e) => {
          ...
          try {
            ...
            if (json?.kind === "MovimientosFondosCollectionBackup") {
              ...
              const bundle = docs.find(
                (b: any) => String(b?.docId || "").trim() === docId,
              );
              if (!bundle) {
                throw new Error(
                  `El backup no contiene un documento con docId "${docId}"`,
                );
              }
              const result =
                await MovimientosFondosService.importBundle(bundle);
              setTestResults((prev) => ({
                ...prev,
            ...
            throw new Error(
              "Formato inválido: se esperaba MovimientosFondosExportBundle o MovimientosFondosCollectionBackup",
            );
          } catch (innerError) {
            const message =
  ...
  const handleExportFirestoreCollection = useCallback(
  ...
  const handleImportFirestoreCollection = useCallback(
    async (collectionName: string) => {
      ...
      try {
        ...
        fileInput.onchange = async (e) => {
          ...
          try {
            ...
            if (
              String(json.collection || "").trim() !==
              String(collectionName || "").trim()
            ) {
              throw new Error(
                `El archivo es de la colección "${json.collection}", no "${collectionName}"`,
              );
            }
  ...
  const uploadImageToFirebase = async () => {
    ...
    try {
      ...
      if (!storage) {
        throw new Error(
          "Firebase Storage no está inicializado. Verifica las variables de entorno.",
        );
      }
        ...
        (error) => {
          ...
          switch (error.code) {
            case "storage/unauthorized":
              errorMessage =
                "Sin permisos para subir archivos. Verifica la configuración de Firebase Storage.";
              break;
            ...
            case "storage/unknown":
              errorMessage =
                "Error desconocido. Verifica la conexión a internet.";
              break;
            ...
            case "storage/bucket-not-found":
              errorMessage =
                "Bucket de Storage no encontrado. Verifica la configuración.";
              break;
  ...
  const testFirebaseConnection = async () => {
    ...
    try {
      ...
      const testData = new Blob(["Firebase connection test"], {
        type: "text/plain",
      });
  ...
  const testExportsFolder = async () => {
    ...
    try {
      ...
      const testData = new Blob(
        [`Test de exports folder - ${new Date().toISOString()}`],
        { type: "text/plain" },
      );
      const uploadTask = uploadBytesResumable(testRef, testData);
  ...
  const deleteSchedulesByempresaAndMonth = async () => {
    ...
    try {
      ...
      // Vista previa de registros a eliminar
      document
        .getElementById("previewDelete")
        ?.addEventListener("click", () => {
          const empresaValue = (
            document.getElementById("empresaSelect") as HTMLSelectElement
          ).value;
          const year = parseInt(
          ...
          const recordsToDelete = allSchedules.filter(
            (schedule) =>
          ...
          if (recordsToDelete.length === 0) {
            if (previewContent) {
              previewContent.innerHTML =
                '<p style="color: #6b7280; margin: 0;">No se encontraron registros con los criterios seleccionados.</p>';
            }
      ...
      document
        ...
        ?.addEventListener("click", async () => {
          ...
          const confirmed = confirm(
            `¿Estás ABSOLUTAMENTE SEGURO que quieres eliminar todos los horarios de ${empresaValue} del mes ${month}/${year}?\n\nEsta acción NO se puede deshacer.`,
          );
          if (!confirmed) return;
          ...
          try {
            ...
            setTestResults((prev) => ({
              ...
              "delete-errors":
                errorCount > 0
                  ? `⚠️ Errores: ${errorCount} registros no pudieron eliminarse`
                  : "",
  ...
  const exportSchedulesWithFilters = async () => {
    ...
    try {
      ...
      (document.getElementById("dateFrom") as HTMLInputElement).valueAsDate =
        lastMonth;
      (document.getElementById("dateTo") as HTMLInputElement).valueAsDate =
        endOfLastMonth;

      // Event listeners
      document
        .getElementById("selectallEmpreas")
        ?.addEventListener("click", () => {
          document
            .querySelectorAll('#empresaFilters input[type="checkbox"]')
            .forEach((cb) => ((cb as HTMLInputElement).checked = true));
        });

      document
        .getElementById("clearallEmpreas")
        ?.addEventListener("click", () => {
          document
            .querySelectorAll('#empresaFilters input[type="checkbox"]')
      ...
      document
        ...
        ?.addEventListener("click", async () => {
          ...
          const dateTo = (document.getElementById("dateTo") as HTMLInputElement)
            .value;
          const selectedempresas = Array.from(
            document.querySelectorAll(
              '#empresaFilters input[type="checkbox"]:checked',
            ),
          ...
          const exportData = {
            ...
            empresas: allEmpreas.filter((loc) =>
              selectedempresas.includes(loc.name),
            ),
          };
      ...
      fileInput.onchange = async (e) => {
        ...
        try {
          ...
          const duplicateCheck =
            importData.schedules.length > 0
              ? await checkForDuplicates(importData.schedules)
              : { duplicates: 0, news: 0 };
      ...
      if (config) {
        if (config.configured) {
          ...
          setTestResults((prev) => ({
            ...
            "email-config-help":
              "💡 Verifica las variables GMAIL_USER y GMAIL_APP_PASSWORD en .env.local",
          }));
        }
      }
  ...
  const sendCustomEmail = async () => {
    ...
    try {
      ...
      const emailMessageInput = document.getElementById(
        "customEmailMessage",
  ...
  const testSections = [
      ...
      tests: [
        {
          ...
          name: "Conexión Firebase",
          action: testFirebaseConnection,
        },
        {
          id: "test-exports",
          name: "Probar /exports/",
          action: testExportsFolder,
        },
    ...
    },
    {
      ...
      tests: [
        {
          id: "export-schedules-filters",
          name: "Exportar con Filtros",
          action: exportSchedulesWithFilters,
        },
        {
          id: "import-schedules",
          name: "Importar Horarios",
          action: importSchedulesFromFile,
        },
        {
          id: "delete-schedules-filter",
          name: "Eliminar por Ubicación/Mes",
          action: deleteSchedulesByempresaAndMonth,
        },
    ...
    },
    {
      ...
      color: "emerald",
      description:
        "Envío de correos electrónicos mediante Gmail con configuración anti-spam",
      tests: [
        {
          id: "email-config",
          name: "Verificar Configuración",
          action: testEmailConfiguration,
        },
        {
    ...
    <div className="max-w-7xl mx-auto bg-[var(--card-bg)] rounded-lg shadow p-6">
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-[var(--muted-foreground)] max-w-2xl mx-auto">
          Sistema de testing y validación de funcionalidades. Ejecuta pruebas
          para verificar el correcto funcionamiento de los diferentes módulos de
          la aplicación.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-5 gap-6 mb-8">
        {testSections.map((section) => {
          ...
          return (
            <div
              key={section.id}
              className="bg-[var(--input-bg)] rounded-lg border border-[var(--border)] p-6"
            >
              <div className="flex items-center mb-4">
                <div
                  className={`w-12 h-12 rounded-lg flex items-center justify-center ${colors.bg} mr-4`}
                >
      ...
      <div className="bg-[var(--input-bg)] rounded-lg border border-[var(--border)] p-6 mb-8">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-4">
          <div>
            ...
            <p className="text-sm text-[var(--muted-foreground)]">
              Consulta la colección MovimientosFondos y descarga los movimientos
              en formato JSON por cada empresa.
            </p>
          </div>
          <button
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {movimientosCompanies.map((company) => {
              ...
              return (
                <div
                  key={company.id}
                  className="p-4 rounded-lg border border-[var(--border)] bg-[var(--background)] flex flex-col"
                >
                  ...
                  <div className="mt-auto grid grid-cols-2 gap-2">
                    ...
                    <button
                      onClick={() =>
                        handleImportMovimientosCompany(
                          company.id,
                          company.company || company.id,
                        )
                      }
      ...
      <div className="bg-[var(--input-bg)] rounded-lg border border-[var(--border)] p-6 mb-8">
        <div className="mb-4">
          ...
          <p className="text-sm text-[var(--muted-foreground)]">
            Exporta o importa cada colección en JSON. Para{" "}
            <span className="font-medium">MovimientosFondos</span> se incluye la
            subcolección <span className="font-medium">movements</span>.
          </p>
        </div>
          ...
          <div className="space-y-4">
            <div className="p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg">
              <h4 className="font-semibold text-red-700 dark:text-red-400 mb-2">
                Estado Actual de Firebase Storage
              </h4>
...
GMAIL_APP_PASSWORD=abcd-efgh-ijkl-mnop`}
        ...
        <div className="mt-6 p-4 bg-gray-50 dark:bg-gray-900/20 border border-gray-200 dark:border-gray-800 rounded-lg">
          <h4 className="font-semibold text-gray-700 dark:text-gray-400 mb-2">
      ...
      {Object.keys(testResults).length > 0 && (
        <div className="mt-6 bg-[var(--input-bg)] rounded-lg border border-[var(--border)] p-6">
          <h3 className="text-lg font-semibold text-[var(--foreground)] mb-4">
            📊 Resumen de Resultados ({Object.keys(testResults).length} pruebas
            ejecutadas)
          </h3>
          <div className="space-y-2 max-h-60 overflow-y-auto">
            {Object.entries(testResults).map(([testId, result]) => (
              <div
                key={testId}
                className="bg-[var(--background)] p-3 rounded-lg border border-[var(--border)]"
              >
