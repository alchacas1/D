...
const PERMISSION_DESCRIPTIONS = {
  ...
  fondogeneralBCR:
    "Permite registrar movimientos del fondo general para la cuenta BCR",
  fondogeneralBN:
...
export default function UserPermissionsManager({
  ...
  const [permissions, setPermissions] =
    useState<UserPermissions>(getNoPermissions());
  ...
  const loadLocations = React.useCallback(async () => {
    try {
      ...
      if (currentUser && currentUser.role !== "superadmin") {
        const allowed = new Set(actorOwnerIds.map((id) => String(id)));
        if (allowed.size > 0) {
          filtered = empresasData.filter(
            (empresa) =>
              empresa &&
              empresa.ownerId &&
              allowed.has(String(empresa.ownerId)),
          );
        }
      }
      setEmpresas(
      ...
      console.error(
        "Error loading empresas from DB, using fallback list:",
        error,
      );
  ...
  const loadEmpresas = React.useCallback(async () => {
    try {
      ...
      console.error(
        "Error loading empresas, using fallback empty list:",
        error,
      );
      setEmpresas([]);
    }
  ...
  useEffect(() => {
    if (userId && users.length > 0) {
      ...
      if (found) {
        ...
        setMessage({
          type: "error",
          text: "No tienes permiso para ver ese usuario o no existe.",
        });
      }
    }
  }, [userId, users]);
  ...
  const handleEmpresaChange = (empresaName: string, isSelected: boolean) => {
    setPermissions((prev) => {
      ...
      const newList = isSelected
        ? [...current, empresaName]
        : current.filter((e) => e !== empresaName);
      return {
        ...prev,
  ...
  const ensureAllPermissions = async () => {
    ...
    try {
      ...
      setMessage({
        type: "error",
        text: "Error al actualizar permisos de usuarios",
      });
    } finally {
      setLoading(false);
    }
        ...
        <div className="space-y-3">
          <div>
            <p className="text-sm text-[var(--muted-foreground)] mb-2">
              Agrega permisos predeterminados a usuarios que no los tienen
              configurados.
            </p>
            <button
          ...
          <div>
            <p className="text-sm text-[var(--muted-foreground)] mb-2">
              Actualiza todos los usuarios para asegurar que tengan todos los
              permisos disponibles (útil cuando se agregan nuevos permisos).
            </p>
            <button
        ...
        <>
          ...
          <div className="mb-6">
            ...
            <div className="mt-2 text-xs text-gray-600">
              <strong>
                Permisos predeterminados para {selectedUser.role}:
              </strong>
          ...
          <div className="mb-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {Object.entries(PERMISSION_LABELS).map(([key, label]) => (
                <div
                  key={key}
                  className="flex items-start p-3 border border-[var(--border)] rounded"
                >
                  <input
                    type="checkbox"
                    id={key}
                    checked={Boolean(
                      permissions[key as keyof typeof PERMISSION_LABELS],
                    )}
                    onChange={(e) =>
          ...
          {permissions.scanhistory && (
            <div className="mb-6 p-4 bg-[var(--muted)] border border-[var(--border)] rounded">
              ...
              <p className="text-sm text-[var(--muted-foreground)] mb-4">
                Selecciona las locaciones específicas a las que este usuario
                tendrá acceso en el historial de escaneos. Si no se selecciona
                ninguna, tendrá acceso a todas las locaciones.
              </p>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {empresas.map((empresa) => (
                  <div
                    key={empresa.id || empresa.name}
                    className="flex items-center p-2 border border-[var(--border)] rounded"
                  >
                    <input
                      type="checkbox"
                      id={`empresa-${empresa.name}`}
                      checked={
                        permissions.scanhistoryEmpresas?.includes(
                          empresa.name,
                        ) || false
                      }
                      onChange={(e) =>
                        handleEmpresaChange(empresa.name, e.target.checked)
                      }
                      className="mr-3 h-4 w-4 text-[var(--primary)] focus:ring-[var(--primary)] border-[var(--border)] rounded"
                    />
                    <label
                      htmlFor={`empresa-${empresa.name}`}
              ...
              {permissions.scanhistoryEmpresas &&
                permissions.scanhistoryEmpresas.length > 0 && (
                  <div className="mt-3 p-2 bg-green-50 border border-green-200 rounded">
                    <p className="text-sm text-green-700">
                      <strong>Empresas seleccionadas:</strong>{" "}
                      {permissions.scanhistoryEmpresas.join(", ")}
                    </p>
                  </div>
                )}
            </div>
