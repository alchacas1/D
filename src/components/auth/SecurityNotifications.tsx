...
export default function SecurityNotifications() {
  ...
  useEffect(() => {
    const checkSecurityEvents = () => {
      ...
      if (isSuperAdmin()) {
        ...
        if (newLocation) {
          const notificationId = `new_location_${Date.now()}`;
          newNotifications.push({
            id: notificationId,
            type: "info",
            title: "Acceso desde Nueva Ubicación",
            message:
              "Se detectó un acceso desde una nueva ubicación geográfica. Si fuiste tú, puedes ignorar este mensaje.",
            timestamp: new Date().toISOString(),
            actions: [
              {
                label: "No Fui Yo",
              ...
              {
                label: "Confirmar",
                action: () => dismissNotification(notificationId),
                type: "primary",
              },
            ],
            requiresSuperAdmin: true,
  ...
  }, [user, isSuperAdmin, logout, dismissNotification]);
  ...
  const visibleNotifications = notifications.filter(
    (notification) => !notification.requiresSuperAdmin || isSuperAdmin(),
  );

  const persistentNotifications = visibleNotifications.filter(
    (n) => n.persistent,
  );
