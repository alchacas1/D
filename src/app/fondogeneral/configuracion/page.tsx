...
type AccountStateMap = Record<
  MovementAccountKey,
  Record<MovementCurrencyKey, CurrencyState>
>;
...
type SnapshotState = Record<
  MovementAccountKey,
  Record<MovementCurrencyKey, SnapshotCurrencyState>
>;

const createCurrencyState = (): CurrencyState => ({
  enabled: true,
  initial: "0",
  current: "0",
...
export default function FondoGeneralConfigurationPage() {
  ...
  const [accountSettings, setAccountSettings] = useState<AccountStateMap>(() =>
    createAccountState(),
  );
  const [snapshot, setSnapshot] = useState<SnapshotState>(() =>
    createSnapshotState(),
  );
  const [storage, setStorage] = useState<MovementStorage<FondoEntry> | null>(
    null,
  );
  ...
  const loadCompanyStorage = useCallback(
    async (
      companyName: string,
    ): Promise<MovementStorage<FondoEntry> | null> => {
      ...
      try {
        ...
        console.error(
          "Error reading Fondo General configuration from Firestore:",
          remoteError,
        );
      }
      ...
      if (!resolved && typeof window !== "undefined") {
        ...
        if (raw) {
          try {
            ...
            console.error(
              "Error parsing Fondo General configuration from localStorage:",
              parseError,
            );
          }
        }
  ...
  const handleToggleCurrency = useCallback(
    (accountId: MovementAccountKey, currency: MovementCurrencyKey) => {
      setAccountSettings((prev) => {
        ...
        if (!nextEnabled) {
          const fallbackCurrency: MovementCurrencyKey =
            currency === "CRC" ? "USD" : "CRC";
          if (!account[fallbackCurrency].enabled) {
            setWarning(
              `Debe quedar activa al menos una moneda en ${ACCOUNT_LABELS[accountId]}.`,
            );
            return prev;
          }
        }
  ...
  const handleToggleCurrentEditing = useCallback(
    (accountId: MovementAccountKey, currency: MovementCurrencyKey) => {
      setAccountSettings((prev) => {
        const account = prev[accountId];
        if (!account) return prev;
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      const updatedBalances = updatedStorage.state.balancesByAccount.map(
        (balance) => {
      ...
      const updatedAccounts = updatedStorage.configuration.accounts.map(
        (account) => {
          const accountState = accountSettings[account.id];
          if (!accountState) return account;
          const supported = ALL_CURRENCIES.filter(
            (currency) => accountState[currency].enabled,
          );
          return {
            ...account,
      ...
      const currencyAvailability: Record<MovementCurrencyKey, boolean> = {
        CRC: false,
        USD: false,
      };
      ...
      const updatedCurrencies = updatedStorage.configuration.currencies.map(
        (currency) => ({
          ...currency,
      ...
      await MovimientosFondosService.saveDocument<FondoEntry>(
        companyKey,
        updatedStorage,
      );
      setStorage(updatedStorage);
      const { settings, snapshot: nextSnapshot } =
        buildStateFromStorage(updatedStorage);
      setAccountSettings(settings);
      ...
      <div className="max-w-4xl mx-auto py-8 px-4">
        <div className="flex flex-col items-center justify-center p-8 bg-[var(--card-bg)] rounded-lg border border-[var(--input-border)] text-center">
          ...
          <p className="text-[var(--muted-foreground)]">
            Esta sección está disponible únicamente para administradores con
            acceso al Fondo General.
          </p>
          <p className="text-sm text-[var(--muted-foreground)] mt-2">
            Contacta a un administrador principal si necesitas habilitar esta
            función.
          </p>
        </div>
      </div>
      ...
      <div className="bg-[var(--card-bg)] border border-[var(--input-border)] rounded-xl p-6">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-sm text-[var(--muted-foreground)]">
            Administra las monedas activas y los saldos iniciales por empresa y
            cuenta. Los cambios impactan las pestañas operativas del Fondo
            General.
          </p>
        </div>
        ...
        {loadingStorage ? (
          ...
          <div className="mt-12 text-sm text-[var(--muted-foreground)]">
            Selecciona una empresa para revisar su configuración del Fondo
            General.
          </div>
        ) : (
              ...
              return (
                ...
                >
                  <div className="flex flex-col gap-1">
                    ...
                    <p className="text-sm text-[var(--muted-foreground)]">
                      Ajusta las monedas disponibles y el saldo inicial base de
                      esta cuenta.
                    </p>
                  </div>
                  <div className="mt-4 grid gap-4 lg:grid-cols-2">
                    {ALL_CURRENCIES.map((currency) => {
                      ...
                      const currentValue = parseInitialValue(
                        currencyState.current,
                      );
                      const formattedBalance = formatByCurrency(
                        currency,
                        currentValue,
                      );
                        ...
                        >
                          <div className="flex items-center justify-between gap-3">
                            ...
                            <button
                              type="button"
                              onClick={() =>
                                handleToggleCurrency(accountId, currency)
                              }
                          ...
                          {currencyState.enabled ? (
                            <div className="mt-3">
                              ...
                              <p className="mt-1 text-[11px] text-[var(--muted-foreground)]">
                                Este valor se usa como punto de partida para el
                                cálculo de saldos en esta moneda.
                              </p>
                              <div className="mt-4">
                                ...
                                />
                                <p className="mt-1 text-[11px] text-[var(--muted-foreground)]">
                                  Ajusta el saldo actual manualmente cuando
                                  requieras corregir discrepancias específicas.
                                </p>
                              </div>
                            </div>
                          ) : (
                            <p className="mt-3 text-xs text-[var(--muted-foreground)]">
                              Activa esta moneda para editar el saldo inicial
                              asociado.
                            </p>
                          )}
                        </div>
        ...
        <div className="mt-8 flex flex-wrap justify-end gap-3">
          <button
            ...
            disabled={
              loadingStorage || saving || selectedCompany.trim().length === 0
            }
          >
          ...
          <button
            ...
            disabled={
              !hasChanges || saving || selectedCompany.trim().length === 0
            }
          >
            {saving ? (
