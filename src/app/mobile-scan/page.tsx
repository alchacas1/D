...
function MobileScanContent() {
  ...
  const loadImagesForCode = useCallback(async (barcodeCode: string) => {
    ...
    try {
      ...
      const imageFiles = result.items.filter((item) =>
        item.name.startsWith(barcodeCode),
      );
    ...
    try {
      ...
      input.onchange = async (e) => {
        ...
        try {
          ...
          const detectedFromPhoto: string | null = await new Promise(
            (resolve) => {
          ...
          await uploadBytes(storageRef, file, {
            ...
            ...(numericCodeBU
              ? { customMetadata: { codeBU: numericCodeBU } }
              : {}),
          });
    ...
    async (scannedCode: string, nameForProduct?: string) => {
      ...
      try {
        ...
        const scanData = {
          ...
          ...(pendingCodeBU &&
            /^\d+$/.test(pendingCodeBU) && { codeBU: pendingCodeBU }),
        };
        ...
        setLastScanned((prev) => [
          ...prev.slice(-4),
          {
            code: scannedCode,
            ...(nameForProduct?.trim() && {
              productName: nameForProduct.trim(),
            }),
            ...(ownercompanieToSend && { ownercompanie: ownercompanieToSend }),
            hasImages,
          },
  ...
  useEffect(() => {
    ...
    const updateHistoryWithImages = async () => {
      ...
      const hasChanges = updatedScans.some(
        (scan, index) => scan.hasImages !== lastScanned[index]?.hasImages,
      );
          ...
          <div className="bg-card-bg rounded-lg p-6 w-full max-w-md">
            ...
            <input
              ...
              onKeyDown={(e) => {
                if (
                  e.key === "Enter" &&
                  (!requestProductName || productName.trim()) &&
                  !isDecodingImage
                ) {
                  handleNameSubmit();
