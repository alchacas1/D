...
const menuItems = [
  ...
  },
  {
    ...
    name: "XML",
    icon: FileCode,
    description: "Generar y exportar XML",
...
export default function HomePage() {
  ...
  >(() => {
    ...
    try {
      ...
      if (session.isSpecialUser && session.id === "special-user-sebastian") {
        ...
        return { kind: "ok", user: specialUser };
      }
      return { kind: "invalid", reason: "wrong-user" };
    } catch (error) {
  ...
  const currentUser = sessionStatus.kind === "ok" ? sessionStatus.user : null;
  const isLoading = false;

  useEffect(() => {
    if (sessionStatus.kind !== "ok") {
      router.push("/");
    }
  }, [router, sessionStatus.kind]);
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 w-full max-w-screen-xl px-4">
          {visibleMenuItems.map((item) => (
            ...
            >
              ...
              <span className="text-sm text-[var(--tab-text)] text-center">
                {item.description}
              </span>
          ...
          <div className="bg-[var(--card-bg)] border border-[var(--input-border)] rounded-xl shadow-2xl p-8 max-w-md w-full relative animate-in fade-in zoom-in duration-200">
            ...
            <div className="flex flex-col items-center text-center">
              ...
              <p className="text-sm text-[var(--muted-foreground)] mb-6">
                Estamos trabajando para mejorar esta funcionalidad. Vuelve
                pronto.
              </p>
