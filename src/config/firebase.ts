...
const isConfigValid = Object.values(firebaseConfig).every(
  (value) => value && !value.includes("your-") && !value.includes("123456789"),
);

if (!isConfigValid) {
  console.warn(
    "⚠️ Firebase configuration contains placeholder values. Please update .env.local with your actual Firebase config.",
  );
}
...
export const db = (() => {
  ...
  try {
    ...
    console.warn(
      "⚠️ Firestore persistent cache unavailable; falling back to memory cache.",
      err,
    );
    const settings = {
