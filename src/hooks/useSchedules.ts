...
export function useSchedules(
  ...
  const fetchSchedules = useCallback(async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      const data = await SchedulesService.getSchedulesByLocationYearMonth(
        locationValue,
        year,
        month,
      );
  ...
  const addSchedule = useCallback(
    async (schedule: Omit<ScheduleEntry, "id" | "createdAt" | "updatedAt">) => {
      try {
        setError(null);
  ...
  const updateSchedule = useCallback(
    async (id: string, schedule: Partial<ScheduleEntry>) => {
      try {
        setError(null);
  ...
  const updateScheduleShift = useCallback(
    async (
      locationValue: string,
      employeeName: string,
      year: number,
      ...
      try {
        ...
        setError(
          err instanceof Error ? err.message : "Error updating schedule shift",
        );
        throw err;
      }
    },
