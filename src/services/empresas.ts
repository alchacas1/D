...
export class EmpresasService {
  ...
  private static normalizeEmpleado(raw: unknown): EmpresaEmpleado {
    ...
    if (!ccss && typeof obj.ccssType === "string")
      ccss = (obj.ccssType as string).trim();
  ...
  private static normalizeEmpleados(rawArr: unknown): EmpresaEmpleado[] {
    ...
    return (rawArr as unknown[]).map((r) =>
      EmpresasService.normalizeEmpleado(r),
    );
  }
  ...
  static async addEmpresa(
      ...
      try {
        ...
        if (owner && typeof owner.maxCompanies === "number") {
          ...
          if (currentCount >= owner.maxCompanies) {
            throw new Error(
              `El dueño de tu empresa ha alcanzado el máximo de empresas permitidas, max: (${owner.maxCompanies})`,
            );
          }
        }
      } catch (err) {
    ...
    const empleadosToSave = EmpresasService.normalizeEmpleados(
      empresa.empleados || [],
    );
