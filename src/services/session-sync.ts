...
export class SessionSyncService {
  ...
  static async hasMobileConnection(sessionId: string): Promise<boolean> {
    try {
      ...
      return activeSessions.some(
        (session) => session.source === "mobile" && session.status === "active",
      );
    } catch (error) {
  ...
  static async hasPCConnection(sessionId: string): Promise<boolean> {
    try {
      const activeSessions = await this.getActiveSessions(sessionId);
      return activeSessions.some(
        (session) => session.source === "pc" && session.status === "active",
      );
    } catch (error) {
  ...
  static subscribeToSessionStatus(
    ...
    try {
      ...
      const unsubscribe = onSnapshot(
        q,
        (querySnapshot) => {
          const sessions: SessionStatus[] = querySnapshot.docs.map(
            (doc) =>
              ({
                id: doc.id,
                ...doc.data(),
  ...
  static createHeartbeatManager(
    ...
    const start = async () => {
      try {
        // Registrar sesión
        sessionDocId = await this.registerSession(
          sessionId,
          source,
          userId,
          userName,
