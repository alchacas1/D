...
export class EmpleadosService {
  ...
  private static normalizeEmpleadoDoc(
    raw: unknown,
    empresaId: string,
  ): Omit<Empleado, "id"> {
    const obj =
    ...
    const nombre = String(
      (obj as any).Empleado ?? (obj as any).name ?? "",
    ).trim();
    const ccssRaw = String((obj as any).ccssType ?? "").trim();
    const ccssType: "TC" | "MT" | "PH" =
      ccssRaw === "MT" || ccssRaw === "TC" || ccssRaw === "PH"
        ? (ccssRaw as "TC" | "MT" | "PH")
        : "TC";
    return {
      empresaId,
