...
class ScanningCache {
  ...
  static getCachedScans(): ScanResult[] | null {
    if (
      this.scansCache &&
      Date.now() - this.scansCacheTime < this.CACHE_DURATION
    ) {
  ...
  static getCachedImageList(): string[] | null {
    if (
      this.imageListCache &&
      Date.now() - this.imageListCacheTime < this.CACHE_DURATION
    ) {
...
export class ScanningService {
   ...
   */
  static async checkMultipleCodesHaveImages(
    codes: string[],
  ...
  static subscribeToScans(
    ...
    try {
      ...
      const unsubscribe = onSnapshot(
        q,
        (querySnapshot) => {
          const scans: ScanResult[] = querySnapshot.docs.map(
            (doc) =>
              ({
                id: doc.id,
                ...doc.data(),
  ...
  static async getCodeBUForCode(barcodeCode: string): Promise<string | null> {
    try {
      ...
      const codeBU =
        (meta.customMetadata &&
          (meta.customMetadata as Record<string, string>).codeBU) ||
        null;
      return codeBU || null;
    } catch (error) {
