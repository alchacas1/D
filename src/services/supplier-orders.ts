...
export class SupplierOrdersService {
  ...
  static async fetchOrdersForCompany(
    companyName: string,
  ): Promise<SupplierOrdersDocument[]> {
    ...
    return documents.map((doc) => {
      ...
      const orders: SupplierOrderEntry[] = rawOrders.map((item: any) =>
        this.mapOrder(item, supplierName, company),
      );
  ...
  static async saveOrder(options: SaveOrderOptions): Promise<string> {
    const { companyName, supplierName, order, userId, previousDocumentId } =
      options;
    ...
    const orderToPersist = this.prepareOrderForStorage(
      order,
      supplierName,
      companyName,
      userId,
    ...
    const existingDoc = await FirestoreService.getById(
    ...
    if (!existingDoc) {
      ...
      const rawOrders = Array.isArray(existingDoc.orders)
        ? existingDoc.orders
        : [];
      const orderIndex = rawOrders.findIndex(
        (item: any) => item.id === orderToPersist.id,
      );
  ...
  private static mapOrder(
    ...
    const products: SupplierOrderProduct[] = productsRaw.map(
      (product: any) => ({
        id:
  ...
  private static prepareOrderForStorage(
    ...
    const total =
      order.total !== undefined && order.total !== null
        ? Number(order.total)
        : totalFromProducts > 0
          ? Number(totalFromProducts.toFixed(2))
          : undefined;
