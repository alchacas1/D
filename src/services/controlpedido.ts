  ...
  return {
    company:
      typeof data.company === "string" && data.company.trim()
        ? data.company.trim()
        : company,
    weekStartKey: asFiniteNumber(data.weekStartKey) ?? weekStartKey,
    entries,
...
export class ControlPedidoService {
  ...
  static async deleteByProviderAndReceiveDateKey(
    ...
    const docRef = doc(
      db,
      COLLECTION_NAME,
      weekDocId(trimmedCompany, receiveWeekStartKey),
    );
    let removedCount = 0;
    ...
    await runTransaction(db, async (tx) => {
      ...
      const existing = normalizeWeekDoc(
        snap.data(),
        trimmedCompany,
        receiveWeekStartKey,
      );
    ...
    holder.unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        const entries = snapshot.exists()
          ? normalizeWeekDoc(snapshot.data(), trimmedCompany, weekStartKey)
              .entries
          : [];
  ...
  static async addEntry(
    ...
    if (
      !Number.isFinite(payload.createDateKey) ||
      !Number.isFinite(payload.receiveDateKey)
    ) {
      throw new Error("Fechas inválidas.");
    }
    if (payload.receiveDateKey < payload.createDateKey) {
      throw new Error(
        "La fecha de recepción no puede ser anterior a la fecha de creación.",
      );
    }
  ...
  static async getWeek(
    ...
    const docRef = doc(
      db,
      COLLECTION_NAME,
      weekDocId(trimmedCompany, weekStartKey),
    );
