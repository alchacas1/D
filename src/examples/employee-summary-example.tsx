...
export default function EmployeeSummaryExample() {
  ...
  const {
    ...
    error: employeeError,
  } = useEmployeeInfo(selectedEmployee, selectedLocation);
    ...
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
        <h1 className="text-2xl font-bold mb-6">
          Ejemplo: Calculadora de Resumen de Empleado
        </h1>
        ...
        <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4 mb-6">
          <h3 className="font-semibold mb-3">
            Información del Empleado desde BD:
          </h3>
          {employeeLoading && (
            <p className="text-sm">Cargando información del empleado...</p>
          )}
          {employeeError && (
            <p className="text-sm text-red-600">Error: {employeeError}</p>
          )}
          {employee && (
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <span
                  className={`ml-1 font-medium ${employee.ccssType === "TC" ? "text-blue-600" : "text-green-600"}`}
                >
                  {employee.ccssType === "TC"
                    ? "Tiempo Completo"
                    : "Medio Tiempo"}
                </span>
              </div>
              <div>
                <span className="font-medium">Horas por turno:</span>{" "}
                {employee.hoursPerShift || 8}
              </div>
              <div>
                <span className="font-medium">Monto extra:</span> ₡
                {employee.extraAmount || 0}
              </div>
            </div>
          )}
          {!employee && !employeeLoading && !employeeError && (
            <p className="text-sm text-gray-600">
              Empleado no encontrado en la ubicación especificada
            </p>
          )}
        </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
        <h2 className="text-xl font-bold mb-4">
          Cómo usar el componente actualizado
        </h2>
...
/>`}
          ...
          <div>
            <h4 className="font-semibold mb-2">
              2. Hook para obtener información del empleado:
            </h4>
            <pre className="bg-gray-100 dark:bg-gray-900 p-3 rounded text-xs overflow-x-auto">
          ...
          <div>
            <h4 className="font-semibold mb-2">
              3. Función utilitaria para cálculo directo:
            </h4>
            <pre className="bg-gray-100 dark:bg-gray-900 p-3 rounded text-xs overflow-x-auto">
  ...
  employee
          ...
          <div>
            ...
            <ul className="list-disc list-inside space-y-1 text-xs ml-4">
              <li>
                <strong>Horarios:</strong> Se obtienen desde la colección
                &lsquo;schedules&rsquo; filtrados por empleado, ubicación, año y
                mes
              </li>
              <li>
                <strong>Configuración CCSS:</strong> Se obtiene desde la
                colección &lsquo;ccss-config&rsquo; con valores MT y TC
                actualizados
              </li>
              <li>
                <strong>Información del empleado:</strong> Se obtiene desde la
                colección &lsquo;locations&rsquo; incluyendo tipo CCSS, horas
                por turno y monto extra
              </li>
              <li>
                <strong>Cálculos:</strong> Se realizan automáticamente basados
                en los datos reales de la base de datos
              </li>
            </ul>
          </div>
        </div>
